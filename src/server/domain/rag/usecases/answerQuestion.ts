...
export async function answerQuestionUsecase(
  ...
  if (!questionVector || questionVector.length === 0) {
    ...
    return {
      stream: generateStreamingResponse(chatbot.systemPrompt, params.question, defaultContext),
      retrievedChunkIds: [],
    };
  }
  ...
  return {
    stream: generateStreamingResponse(chatbot.systemPrompt, params.question, contextText),
    retrievedChunkIds,
  };
}
